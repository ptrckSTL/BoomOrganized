...
class GoogleSheetSelectionContract : ActivityResultContract<Unit, UserSheet?>() {

    override fun createIntent(context: Context, input: Unit): Intent {
        return GoogleSheetSelectionActivity.createIntent(context)
    }

    override fun parseResult(resultCode: Int, intent: Intent?) = intent?.getParcelableExtra<UserSheet?>(GOOGLE_SHEET_RESULT)
}
